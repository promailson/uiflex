...
$('#btnS4').click(function() {
    ...
    chrome.storage.sync.get(function(data) {
        if (data.visao1 == 1 && data.visao2 == 1 && data.visao3 == 1) {
            chrome.storage.sync.set({
                'mailson2': 2
            });
        }
...
$('#btnS5').click(function() {
    ...
    chrome.storage.sync.get(function(data) {
        if (data.daltonismo == 1 && data.daltonismo2 == 1) {
            chrome.storage.sync.set({
                'mailson3': 2
            });
        }
    ...
    chrome.storage.sync.get(function(data) {
        ...
        $.getJSON("http://slifes.dc.ufscar.br/uiflex/rules.json", function(json) {
            ...
            for (var i in json) {
                ...
                if (json[i].id == "rule13" && json[i].context.user.predicate == "AbilityToSee" && json[i].context.user.object == "low" && (data.visao2 == 2)) {
                    chrome.storage.sync.set({
                        'rule13': '1'
                    });
                }
                if (json[i].id == "rule8" && json[i].context.user.predicate == "AbilityToDifferentiateColors" && json[i].context.user.object == "no" && (data.daltonismo == 2 || data.daltonismo2 == 2)) {
                    chrome.storage.sync.set({
                        'rule8': '1'
                    });
                }
                if (json[i].id == "rule5" && json[i].context.user.predicate == "AbilityToDifferentiateColors" && json[i].context.user.object == "no" && (data.daltonismo == 2 || data.daltonismo2 == 2)) {
                    chrome.storage.sync.set({
                        'rule5': '2'
                    });
                }
                ...
                if (json[i].id == "rule10" && json[i].context.user.predicate == "AbilityToSee" && json[i].context.user.object == "low" && (data.visao1 == 2 || data.visao2 == 2 || data.visao3 == 2)) {
                    chrome.storage.sync.set({
                        'rule10': '1'
                    });
                }
                ...
                if (json[i].id == "rule14" && json[i].context.user.predicate == "AbilityToSee" && json[i].context.user.object == "medium" && (data.visao3 == 2)) {
                    chrome.storage.sync.set({
                        'rule14': '1'
                    });
                }
                ...
                if (json[i].id == "rule35" && json[i].context.user.predicate == "AbilityToDifferentiateColors" && json[i].context.user.object == "no" && (data.daltonismo == 2 || data.daltonismo2 == 2)) {
                    chrome.storage.sync.set({
                        'rule35': '1'
                    });
                }
